rsInference <- function(rsfit, efficient = FALSE){
  ...
  ESX <- apply(covariate[splitIndex$fit,], 2, function(t){exp_gam(link[splitIndex$fit], estimatedNuisance$S[splitIndex$fit] * t)})
  ES <- exp_gam(link[splitIndex$fit], estimatedNuisance$S[splitIndex$fit])
  ...
  # return
  list(fit=rsfit$fit, betaAN=betaAN, W1=W1, W2=W2)
}
